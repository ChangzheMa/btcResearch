...
def cal_factor_by_ask_bid(factor_name: str, asks: list[Order], bids: list[Order]):
    if factor_name.startswith('lob_vol_ib'):
        idx = int(factor_name[10:]) - 1
...
def read_file_to_csv(file_path, factor_name_list=None):
    if factor_name_list is None:
        factor_name_list = get_lob_factor_name_list()
    ...
    for line in lines:
        ...
        row = [math.floor(data_obj['current']) + 1]
        for factor_name in factor_name_list:
            row.append(cal_factor_by_ask_bid(factor_name, data_obj['asks'], data_obj['bids']))
        row_list.append(row)
    return pd.DataFrame(row_list, columns=['timestamp_sec', *factor_name_list])


# 读取和计算 lob 的数据
lob_column_names = get_lob_column_names()
lob_files = [file for file in os.listdir('../gate/lobsli/')]
lob_files.sort()

lob_factor_name_list = get_lob_factor_name_list()
lob_dfs = [read_file_to_csv(f"../gate/lobsli/{file}", lob_factor_name_list) for file in lob_files if file.startswith('BTC_USDT')]
lob_df = pd.concat(lob_dfs)

# 读取和计算 trade 数据
column_names = get_trade_column_names()
df = pd.read_csv('../gate/BTC_USDT-202408.csv', names=column_names)
...
df_filled['price'] = df_filled['price'].ffill()

# pre_sec_count_list = [5, 10, 20, 40, 80, 160, 320, 640]   # 5秒 ~ 10分钟
pre_sec_count_list = [5, 10, 20, 40, 80]   # 5秒 ~ 80秒
factor_list = []
...
# 把 trade 数据和 lob 数据合并起来，然后计算 corr
df_sampled = df_filled
df_sampled = pd.merge(df_sampled, lob_df, on='timestamp_sec')

factor_list = factor_list + lob_factor_name_list
